<div id="form-module-wrapper" class="sec-wrapper no-border clearfix" data-ytkey="<?=$this->site_config['youtube_api_key']?>">
	<?php if(isset($data)) {?>
		<form class="form-module" action="<?=$this->admin_url?>update_module" method="POST">
			...
			<?if(isset($category)){
				...
				$this->load->view('admin/blocks/append/module_tree', $category_data);
			}?>
			<?if($data->m_meta != ''){?>
				<?$meta = json_decode($data->m_meta, true);?>
				<? switch($data->mc_slug){
					case 'category-tree':
						$this->load->view('admin/blocks/append/single_value');
						$category_data = array(
							'default_tree_id' => $meta['id'],
							'default_tree_name' => $meta['name'],
							'category' => $category_tree
						);
						$this->load->view('admin/blocks/append/category_tree', $category_data);
						break;
					default:
						$this->load->view('admin/blocks/append/single_value', array('default_single_name' => $meta['val']));
	...
	<?php }
		...
		<form class="form-module" action="<?=$this->admin_url?>add_module" method="POST">
			...
			<? $this->load->view('admin/blocks/append/module_tree')?>
			<?$this->load->view('admin/blocks/append/single_value');?>
			<?$this->load->view('admin/blocks/append/category_tree', array('category' => $category_tree));?>
			<div id="adordering-wrapper" class="col-xs-12 form-group-custom">
				<label class="col-xs-12 col-sm-2">Ordering:</label>
